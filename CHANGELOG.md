...
- **Host-declared configuration (`manifest_version: 2`)**: `plugin.yaml` declares a `config_schema` for all 24 settings, so Hermes renders them in Desktop/CLI Plugins panels and type-checks them at load. Effective settings resolve per key: `ctx.get_config()` > host `plugins.entries.hermes-auto-titler.settings` > plugin `config.yaml` (legacy 0.2 files keep their explicitly-set keys) > built-in defaults. A legacy `config.yaml` that tuned one key is never wholesale overridden by host defaults. One coercion path serves the YAML entry, `/autotitler config`, and host type checks. The model override is declared as `title_model` because the host reserves the bare `model` settings root; it normalizes back to the internal `model` key on load and `config.yaml` keeps writing `model`.
- **Compaction summary preview**: Trim compaction summaries by whole Markdown sections so a long summary keeps both the leading arc and trailing `## User Messages` section; avoid duplicating opening/recent turns when compaction leaves fewer visible turns than `recent_turns`. Templates without Markdown headings retain the sentence-boundary fallback, which keeps a section's heading and body together. `include_compacted` remains unused because the summary is already visible and un-compacting it would re-inflate the title prompt.
- **Catalog sync release gate**: `scripts/check_catalog_sync.py` compares the local `plugin.yaml` version and tagged commit against the published plugin-catalog entry, rejecting missing/stale entries and short or invalid SHAs. It runs only as a manually dispatched release job, since the matching tag and published entry must exist before the gate can pass.
