...
- **来源安全**：遵守 `derived < llm < user`，保护用户手改标题和旧 legacy 标题
...
- **Profile 与记账完整**：后台线程继承当前 Hermes profile Context，真实调用写入原生 auxiliary usage
...
- Hermes 内部执行不计入轮数：`bg-review` 后台回顾线程与 `platform=cron|subagent`（大小写不敏感）的轮次直接忽略；被打断/失败/未完成（completed=False / failed / interrupted，即使标志不一致）的轮次同样不计
- 真实会话关闭/终局走 `on_session_finalize` lifecycle hook（或带非空 reason 的 `on_session_end`），正常节流（force=False），不重复进行中的自动评估；关闭评估刻意保持同步——可能阻塞至 provider timeout（最长 ~30s），换取关闭前完成标题更新的确定性
...
- 判断模型独立于主对话（默认宿主模型，可配 deepseek-v4-flash），输出 `keep` / `rename` JSON；插件请求 `max_tokens=64`，但是否成为 wire 级硬上限取决于 Hermes/provider
- 写回遵守 Hermes 标题来源优先级：每次写尝试前都重新核对来源，正常路径优先保护用户手改标题；`derived` 是首条用户消息的确定性临时兜底，插件评估时始终要求升级为正式模型标题（不只处理超长首句）；自动标题保持可升级。`llm → llm` 因宿主公开 API 没有同级原子 CAS，只能临时 user 写入再恢复 llm 来源，最后仍有极小的跨步骤竞态窗口，不宣称绝对原子或绝对零覆盖
...
cp <repo>/config.yaml.example hermes-auto-titler/config.yaml             # 或让安装器自动从 config.yaml.example 生成
```
...
| `recent_turns` | `2` | 每次评估携带最近 N 个真实用户轮次；每轮最多保留用户消息 + 最后一条模型文本回复（重验证后维持 2：1/1 无优势，且更容易被结尾子任务带偏） |
...
~/.hermes/hermes-agent/venv/bin/python scripts/integration_check.py   # 临时 SessionDB 集成验证
~/.hermes/hermes-agent/venv/bin/python scripts/e2e_check.py <session_id>  # 真实模型、写回与 usage 验证
```
