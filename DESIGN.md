...
{rule}                    ← 策略段（conservative / aggressive / blind）
{style_req}               ← 风格段（concise / complete）
最终标题长度不超过 {max_title_length} 字符（完整脉络也不能超）。
{冗长阈值}                 ← concise: 超 40 字符视为冗长；complete: 40~60 合理，超 60 压缩

[user]
当前标题：xxx              ← 非 blind 模式才给（避免旧标题引导模型）
会话开头：                 ← opening 轮（每条 preview_chars 字符）
最近对话：                 ← recent 轮（每条 preview_chars 字符）
会话中的全部用户消息：      ← 意图轨迹（全量不截断）
```
