...
| **可选复审门** | `rename_confirmations: 0` 表示 llm→llm 改名在一次判定后直接写入；`1` 表示候选先挂起，再多做一次评估，由模型 approve、替换或放弃。`renames_per_hour` 另行限制改名频率。 |
...
4. **判断主题，而不是执行手段** —— 辅助模型只返回严格 JSON。提示词优先看持续主题和目标，而不是最新一条消息；代码、日志、shell 命令默认只是上下文，并用反事实规则排除偶然出现的工具，除非工具本身就是正在开发、配置、排障或比较的对象。
5. **可选复审改名** —— `rename_confirmations: 1` 时，llm→llm 改名先进入 pending。下一次评估可 `approve`、给出更好的 `rename`，或 `keep` 当前标题。无标题/derived 升级以及显式 blind 重生成不经过这层。
6. **安全写回** —— 每次写尝试前重新核对 provenance；标题冲突时添加后缀，并继续遵守字符数和显示列宽限制；自动标题写入后保持 `llm` 来源，因此以后用户手改仍然拥有更高优先级。
...
- **为什么提供额外复审？** 单次改名即使看起来合理，也可能造成来回跳动。多一次评估可以让模型重新把候选和整段会话对照后再决定是否写入。
...
| `strategy` | `conservative` | v0.1.3 仍保留的兼容配置。当前 `_generate()` 路径对两个值使用同一套“持续主题优先”判定规则。 |
...
| `rename_confirmations` | `0` | `0` = 一次判定后直接执行 llm→llm 改名；`1` = 再要求一次复审。配置解析目前允许大于 `1`，但实现仍与 `1` 使用同一层复审门。 |
