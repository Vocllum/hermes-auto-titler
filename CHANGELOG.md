...
- **0.3 non-invasive title coexistence**: The plugin no longer writes `auxiliary.title_generation.enabled` — on install, on `first_title_mode` changes, or anywhere else. Loading the plugin leaves the host title generator untouched, so disabling or deleting the plugin directory cannot silently kill Hermes' own titler. `first_title_mode` now defaults to `builtin`: Hermes owns the first-turn title and the plugin starts from its multi-turn evolution, drift and close evaluation. `plugin` remains available as an explicit opt-in, and neither mode touches host configuration. Startup-time ownership changes still report restart-required in `/autotitler config`.
- **Host-declared configuration (`manifest_version: 2`)**: `plugin.yaml` declares a `config_schema` for all 24 settings, so Hermes renders them in Desktop/CLI Plugins panels (24 fields) and type-checks them at load. Effective settings resolve per key: `ctx.get_config()` > host `plugins.entries.hermes-auto-titler.settings` > plugin `config.yaml` (legacy 0.2 files keep their explicitly-set keys) > built-in defaults. Per-key resolution means a legacy `config.yaml` that tuned one key is never wholesale overridden by host defaults. One coercion path (`coerce_value`) serves the YAML entry, `/autotitler config`, and host type checks: the YAML entry falls back to a default on an invalid value while the command entry reports the error. The model override is declared as `title_model` because the host reserves the bare `model` settings root (it would be silently dropped from the panel and rejected by the host's writer); `title_model` normalizes back to the internal `model` key on load and `config.yaml` keeps writing `model`.
- **Catalog sync check**: `scripts/check_catalog_sync.py` compares the local `plugin.yaml` version and its tagged commit against the published plugin-catalog entry, and fails when the entry lags, is missing, or pins a short/non-40-hex sha. It runs as a manually dispatched release job in `.github/workflows/tests.yml` (`catalog-sync`) rather than on every push, because it requires the version's tag to exist and the catalog entry to already be published — running it per-push would fail on any unreleased version bump. An unreachable catalog or a missing tag exits 2 and is never reported as in-sync. Release tooling only; no runtime behavior.
- **Compaction summary preview**: The compaction block is now trimmed by whole markdown sections instead of a linear prefix cut, so a long summary keeps both the leading arc and the trailing `## User Messages` section that a prefix cut silently drops. `opening` and `recent` no longer repeat the same turns when compaction leaves fewer visible turns than `recent_turns`. Sections are located by `^## ` position only, so custom compaction templates keep working; a template with no markdown headings falls back to the previous sentence-boundary preview. `include_compacted` is deliberately not used — the summary is already in the visible stream, and un-compacting it would re-inflate the title prompt by an order of magnitude. When no whole section fits the tail budget, the sentence-boundary fallback previews a single unclaimed section, so its heading line and body always come from the same section instead of gluing one section's title onto another's text.
