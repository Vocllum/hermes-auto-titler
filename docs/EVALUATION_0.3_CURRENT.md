...
基于 `eval/fixtures/manifest.json` 锁定的真实顶层长会话样本（严格剔除 subagent、完成包与系统噪声，且初始状态为可自动命名），通过 `eval/replay.py` 的沙箱时序切片器 `slice_prefix`，在不连网的情况下量化各阶段切片进入 `load_context_with_summary` 的结构表现：

| 会话 ID | 场景分类 | 人类轮次 / 总消息 | 切片轮次 | 切片消息数 | Opening 轮数 | Recent 轮数 | All User 覆盖 | Summary(1200) 字符 | Summary(2400) 字符 | 载体污染与阶段覆盖诊断 |
|---|---|---|---|---|---|---|---|---|---|---|
| `20260920_123615_4302a0` | repeat_compacted | 24 / 781 | Turn 1 | 28 | 1 | 2 | 1 | 999 chars | 1961 chars | 无前置压缩泄漏，完整保留首轮意图 |
...
### 核心离线结论
1. **时序与载体零泄漏**：前缀切片严格阻断了末端压缩向早期的穿越，Turn 1 看到的均为初始未压缩真实消息；
