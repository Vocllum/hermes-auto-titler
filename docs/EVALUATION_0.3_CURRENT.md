...
> **2026-09-26 复核：下文阶段二只可用于网关连通性和调用消耗诊断，标题质量结论无效。** 当前离线 smoke runner 仅开放 `active_window` 活跃视窗诊断；`historical_replay` 因缺失压缩 provenance、撤回消息排除与初始标题来源重建而显式拒绝运行，尚无可信的全历史质量试验。`scripts/run_smoke_trace.py:63-70,96-108` 将原生 `sqlite3.Connection` 传给 `make_sandbox()`；该对象没有 `get_session()` / `get_session_title()`，因此沙箱中的初始标题和来源均为空。脚本还对每个 cell/前缀重新创建沙箱，以 `force=True` 单次调用 `evaluate()`；第 8 轮没有继承第 1 轮的标题、pending、冷却或真实周期 gate，连既有标题的 keep/approve 分支也没有在这组 smoke 中得到有效验证。输入来自 `active=1` 的压缩后视窗，不能宣称这是会话真正的第 1 轮。下文的 `Applied Title` 是单次沙箱结果，不是连续生产标题链；所有覆盖率、篡权率与改名稳定性数字不得作为模型质量结论或调参依据。`eval/metrics.py:46-85` 将不在可接受标题词表内的任意同义标题机械计作“局部篡权”，也不能替代人工盲审。现阶段需按原始事件时序和初始 provenance 重建沙箱，连续触发生产 hook，并逐轮盲审标题后才比较架构／提示词。历史探针记录保留以便审计。
...
3. **旧质量指标已撤销**：模型生成的标题与预置短词表未全词等值匹配，只能证明“未命中这组文字标签”。先前报告的 0.0% 主线覆盖率、100.0% 局部篡权率混淆了等值匹配与语义质量，且基于互不继承状态的单次沙箱调用，均不得用作标题质量或稳定性判断。质量结论待连续状态机回放和逐条盲审。
