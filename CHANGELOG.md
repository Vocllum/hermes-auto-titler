...
- **Catalog sync check**: `scripts/check_catalog_sync.py` compares the local `plugin.yaml` version and its tagged commit against the published plugin-catalog entry, and fails when the entry lags, is missing, or pins a short/non-40-hex sha. Release tooling only; no runtime behavior.
